...
## 80、针对70与75点，还有一种解决办法，就是头文件仅声明，定义放到源文件，这样源文件就可以include apr库了（这种也只能用指针）

有没有办法在头文件声明结构体，在源文件定义结构体呢，这样在源文件里去include apr库的头文件

## 85、整体原则：头文件去typedef成 `base_` 开头的自定义头文件，源文件直接去include apr库使用（能不重定义，就不要浪费时间去自(重)定义）
