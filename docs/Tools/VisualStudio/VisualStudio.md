...
在 Visual Studio 中使用 CMake 工程时，执行 "Clean All" 操作会删除由之前构建生成的所有中间文件和输出文件。这是为了确保一个干净的构建环境，通常在遇到构建问题或想要重新构建整个项目时会使用。

### Clean All 的具体操作如下：

1. **删除生成的对象文件**：删除所有中间文件，如 `.obj` 文件，这些文件是编译源代码时生成的。
2. **删除生成的库和可执行文件**：删除所有生成的目标文件，如 `.lib`、`.dll` 和 `.exe` 文件。
3. **删除中间文件夹**：删除 CMake 在构建过程中创建的中间文件夹（例如 `CMakeFiles` 目录）。
4. **删除构建系统文件**：删除由 CMake 生成的构建系统文件（例如 `Makefile`、`CMakeCache.txt`、`cmake_install.cmake` 等）。
...
在执行 "Clean All" 操作后，`build` 目录中的所有中间文件和生成的目标文件都会被删除。

### 具体步骤

1. **打开 Visual Studio** 并加载你的 CMake 工程。
2. **选择菜单中的 `Build`** 然后点击 `Clean All`。
3. Visual Studio 会显示一个输出窗口，显示删除过程中的详细信息。

### 注意事项

- **清理后需要重新配置和生成**：在执行 "Clean All" 后，你可能需要重新配置和生成 CMake 工程，以重新生成所有必要的文件。
- **确保备份重要文件**：虽然 "Clean All" 主要删除中间和输出文件，但最好确保你的源代码和项目文件已经备份。

通过执行 "Clean All"，你能够确保一个干净的构建环境，避免因为旧的中间文件或输出文件导致的构建问题。
...
可以把项目解决方案下的.vs文件删除掉，然后再重新打开sln解决方案进行编译，有可能可以恢复
...
![image-20250111111409493](resource/img/image-20250111111409493.png)
